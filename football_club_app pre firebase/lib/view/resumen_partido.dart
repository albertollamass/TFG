...
class _ResumenPartidoState extends State<ResumenPartido> {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < equipos.length; i++) {
      ...
      for (int j = 0; j < equipos.values.elementAt(i).length; j++) {
        equiposW.add(Row(children: [
          ...
          goles[0][1][equipos.keys.elementAt(i).toString()].toString() ==  equipos.values.elementAt(i).elementAt(j).toString() ?  Row(children: [Text(1.toString(), style: const TextStyle(fontSize: 20),), const SizedBox(width: 5),const Icon(Icons.sports_soccer, size: 30,)]): const Text("")
        ]));        
        equiposW.add(
          const SizedBox(height: 10,),
        );
    ...
    print(goles[0][1]["negro"].toString());
